...
![Variation of the different equations used to convert the measured SPAD-values to real chlorophyll values.](imgs/SPAD_conversion_variation.png "Variation of the different equations used to convert the measured SPAD-values to real chlorophyll values."){width=80%}
...
Another task was to compare our LMA values with those from the LEDA database (See Figure 2 in this section). There, the values were available as mean SLA, so we converted them into LMA first. If several values were available for one plant, we calculated the mean value. For two plants no values were available in the LEDA database. Overall, our values are slightly higher than those in the database, but are very similar overall.
